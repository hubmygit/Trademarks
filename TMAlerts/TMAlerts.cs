...
namespace TMAlerts
{
    public partial class TMAlerts : Form
    {
        ...
        public List<Task> Check_Tasks()
        {
            ...
            string SelectSt = "SELECT Id, TrademarksId, isnull(TM_StatusId, 0) as TM_StatusId, EventTypesId, ExpDate, AlertDescr " +
                              "FROM [dbo].[Tasks] " +
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    newTask.TM_StatusId = Convert.ToInt32(reader["TM_StatusId"].ToString());
        ...
        public bool SendEmailAlertsPerTask(List<Task> tasks)
        {
            ...
            foreach (Task thisTask in tasks)
            {
                ...
                Trademark tmRec = new Trademark(thisTask.TrademarksId);

                string emailBody = "Trademark " + tmRec.TMNo + ": " + tmRec.TMName + ".\r\n" +
                                   thisTask.EventTypes.Name + ".\r\n";

                if (thisTask.EventTypesId == 1) //ananewsi
                {
                    emailBody += "Χρονική προθεσμία ανανέωσης σήματος.\r\nΠαρακαλώ αν προβείτε σε ανανέωση, ενημερώστε το σύστημα.\r\n";
                }
                else if (thisTask.EventTypesId == 2) //apofasi
                {
                    emailBody += "Χρονική προθεσμία έκδοσης απόφασης.\r\nΠαρακαλώ μετά την έκδοση απόφασης, ενημερώστε το σύστημα.\r\n";
                }
                else if (thisTask.EventTypesId == 3) //oristikopoiisi
                {
                    emailBody += "Χρονική προθεσμία ανακοπών απόφασης: " + Task.getDecisionNo(thisTask.TM_StatusId) + ".\r\nΠαρακαλώ μετά τη λήξη, οριστικοποιείστε αν δεν υπάρχει εκκρεμότητα.\r\n";
                }
                else if (thisTask.EventTypesId == 4) //prosfygi
                {
                    emailBody += "Χρονική προθεσμία προσφυγών απόφασης: " + Task.getDecisionNo(thisTask.TM_StatusId) + ".\r\nΠαρακαλώ αν προβείτε σε προσφυγή, ενημερώστε το σύστημα.\r\n";
                }

                emailBody += ("Υπεύθυνος Δικηγόρος: " + Trademark.getResponsibleName(tmRec.ResponsibleLawyerId) + ".\r\n" +
                             "Εταιρία: " + Trademark.getCompanyName(tmRec.CompanyId) + ".\r\n" +
                             "Εθνική Ισχύς: " + Trademark.getNationalPowerName(tmRec.NationalPowerId) + ".\r\n" +
                             "\r\n" +
                             "Ημερομηνία Κατάθεσης: " + tmRec.DepositDt.ToString("dd.MM.yyyy HH:mm") + ".\r\n");

                //if (tmpRec.RenewalDt != null)
                //{
                //    emailBody += "Ημερομηνία Τελευταίας Ανανέωσης: " + tmpRec.RenewalDt.ToString("dd.MM.yyyy HH:mm") + ".\r\n";
                //}

                DateTime? LastRenewalDt = Task.getLastRenewalDt(thisTask.TrademarksId);
                if (thisTask.EventTypesId == 1 && LastRenewalDt != null) //ananewsi
                {
                    emailBody += "Ημερομηνία Τελευταίας Ανανέωσης: " + ((DateTime)LastRenewalDt).ToString("dd.MM.yyyy HH:mm") + ".\r\n";
                }

                emailBody += ("Καταληκτική Ημερομηνία: " + thisTask.ExpDate.ToString("dd.MM.yyyy HH:mm") + " [" + thisTask.AlertDescr + "]" + ".\r\n" +
                              "\r\n" +
                              //"Url: " + tmpRec.Url + "\r\n" +
                              "Url: " + Task.getUrl(thisTask.TM_StatusId) + "\r\n" +
                              "\r\n\r\n" +
        ...
        /*
        public bool SendEmailAlertsPerTask_Obsolete(List<Task> tasks)
        {
            Output.WriteToFile("Send Email Alerts Per Task");
            bool ret = true;
            ...
            foreach (Task thisTask in tasks)
            {
                ...
                string emailBody = "Trademark " + tmpRec.TMNo + ": " + tmpRec.TMName + ".\r\n" +
                                   thisTask.EventTypes.Name + ".\r\n" +
                                   "\r\n" +
                                   "Υπεύθυνος Δικηγόρος: " + TempRecords.getResponsibleName(tmpRec.ResponsibleLawyerId) + ".\r\n" + 
                                   "Εταιρία: " + TempRecords.getCompanyName(tmpRec.CompanyId) + ".\r\n" + 
                                   "Εθνική Ισχύς: " + TempRecords.getNationalPowerName(tmpRec.NationalPowerId) + ".\r\n" +
                                   "\r\n" +
                                   "Ημερομηνία Κατάθεσης: " + tmpRec.DepositDt.ToString("dd.MM.yyyy HH:mm") + ".\r\n";

                //if (tmpRec.DepositDt != null) 
                //{
                //    emailBody += "Ημερομηνία Τελευταίας Ανανέωσης: " + tmpRec.DepositDt.ToString("dd.MM.yyyy HH:mm") + ".\r\n";
                //}
                if (tmpRec.RenewalDt != null) 
                {
                    emailBody += "Ημερομηνία Τελευταίας Ανανέωσης: " + tmpRec.RenewalDt.ToString("dd.MM.yyyy HH:mm") + ".\r\n";
    ...
    public class Task
    {
        ...
        public static string getDecisionNo(int TM_StatusId)
        {
            string ret = "";
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ret = reader["DecisionNo"].ToString();
                }
                reader.Close();
            }
            catch (Exception ex)
        ...
        public static string getUrl(int TM_StatusId)
        {
            string ret = "";
        ...
        public static DateTime? getLastRenewalDt(int Trademarks_Id)
        {
            DateTime? ret = null;
    ...
    public class TempRecords_Obsolete
    {
        public int Id { get; set; }
        ...
        public TempRecords_Obsolete()
        {
        }

        public TempRecords_Obsolete(int Id)
        {
            SqlConnection sqlConn = new SqlConnection(SqlDBInfo.connectionString);
    ...
    public class Trademark
    {
        ...
        {
            SqlConnection sqlConn = new SqlConnection(SqlDBInfo.connectionString);
            string SelectSt = "SELECT [Id], [TMNo], [DepositDt], [NationalPowerId], [TMGrNo], [CompanyId], [ResponsibleLawyerId], [TMName],  " +
                              "[FileName], [FileContents], [Description] " +
                              "FROM [dbo].[Trademarks] " +
                              "WHERE Id = @Id ";
